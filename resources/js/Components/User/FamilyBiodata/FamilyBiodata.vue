...
<template>
    ...
    <form @submit.prevent="submit" class="grid grid-cols-12 gap-0">
        ...
        <div class="form_item col-span-12 md:col-span-6 p-2">
            <select v-model="form.personal_maritial_agreement" @change="(e) => { single_biodata.personal_maritial_agreement = e.target.value }" id="personal_maritial_agreement" name="personal_maritial_agreement"
                class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
                <option value="null" disabled selected >{{ translations.biodata_form.family_biodata.personal_maritial_agreement_title }}</option>
                <option v-for="(personal_maritial_agreement, personal_maritial_agreement_key) in translations.biodata_form.family_biodata.personal_maritial_agreement_options" :key="personal_maritial_agreement.id" :value="personal_maritial_agreement_key">{{ personal_maritial_agreement }}</option>
            </select>
            <InputError class="mt-2" :message="form.errors.personal_maritial_agreement" />
        </div>

        <div class="form_item col-span-12 md:col-span-6 p-2">
            <select v-model="form.family_maritial_agreement" @change="(e) => { single_biodata.family_maritial_agreement = e.target.value }" id="family_maritial_agreement" name="family_maritial_agreement"
                class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
                <option value="null" disabled selected >{{ translations.biodata_form.family_biodata.family_maritial_agreement_title }}</option>
                <option v-for="(family_maritial_agreement, family_maritial_agreement_key) in translations.biodata_form.family_biodata.family_maritial_agreement_options" :key="family_maritial_agreement.id" :value="family_maritial_agreement_key">{{ family_maritial_agreement }}</option>
            </select>
